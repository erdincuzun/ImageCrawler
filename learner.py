...
class learner:
        ...
        for theDataExtraction in dataExraction_list:
            ...
            if parent_layout == '*':
                ...
                for theRule in extraction_rules:
                    ...
                    check_res, preferred_regex, _ = self.check_regex(rule, html)
                    if check_res:
                        self.ro.data["sitemap"][i]["data_extraction"][j]["extraction_rules"][k]["selector_regex"] = preferred_regex
                    k += 1
            else:
                check_res, preferred_regex, part_html = self.check_regex(parent_layout, html)
                if check_res:
                ...
                for theRule in extraction_rules:
                    result_type = self.ro.get_sm_extraction_result_type(theRule)                   
                    rule = self.ro.get_sm_extraction_rule_selector(theRule, result_type)
                    check_res, preferred_regex, _ = self.check_regex(rule, part_html)
                    if check_res:
                        self.ro.data["sitemap"][i]["data_extraction"][j]["extraction_rules"][k]["selector_regex"] = preferred_regex
                k += 1
            j += 1        
    ...
    def add_web_site_information_to_url(self, new_links):
        for ind in range(len(new_links)):
            if new_links[ind] != None and len(new_links[ind]):
                if not (new_links[ind].startswith("http:") or new_links[ind].startswith("https:")): #add http and web site name to link, (/.../..../) -> http://website/.../..../
    ...
    def check_regex(self, rule, html):
        ...
        if res_rg != None:
            ...
            if generator_regex != preferred_regex: #inner tag is not appropriate, add/update the regex suggestion
                return True, preferred_regex, data_result
            else:
                return False, "", data_result
        else:
            return False, "", ""
